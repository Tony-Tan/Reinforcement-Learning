...
def test_dqn(agent: DQNPPAgent, test_episodes: int):
    """
...
def train_dqn(logger):
    ...
    dqn_pp_agent = DQNPPAgent(cfg['input_frame_width'], cfg['input_frame_height'], env.action_space,
                           cfg['mini_batch_size'],
    ...
    while training_steps < cfg['training_steps']:
        ...
        run_test = False
        reward_cumulated = 0
        obs = dqn_pp_agent.perception_mapping(state, step_i)
        while (not done) and (not truncated):
            #
            if len(dqn_pp_agent.memory) > cfg['replay_start_size'] and step_i >= cfg['no_op']:
                action = dqn_pp_agent.select_action(obs)
            else:
                action = dqn_pp_agent.select_action(obs, RandomAction())
            next_state, reward_raw, done, truncated, inf = env.step(action)
            reward = dqn_pp_agent.reward_shaping(reward_raw)
            next_obs = dqn_pp_agent.perception_mapping(next_state, step_i)
            dqn_pp_agent.store(obs, action, reward, next_obs, done, truncated)
            dqn_pp_agent.train_step()
            obs = next_obs
            reward_cumulated += reward

            if (len(dqn_pp_agent.memory) > cfg['replay_start_size'] and
        ...
        if run_test:
            ...
            avg_reward, avg_steps = test_dqn(dqn_pp_agent, cfg['agent_test_episodes'])
            logger.tb_scalar('avg_reward', avg_reward, epoch_i)
            logger.tb_scalar('avg_steps', avg_steps, epoch_i)
            logger.tb_scalar('epsilon', dqn_pp_agent.exploration_method.epsilon, epoch_i)
            logger.msg(f'{epoch_i} avg_reward: ' + str(avg_reward))
            logger.msg(f'{epoch_i} avg_steps: ' + str(avg_steps))
            logger.msg(f'{epoch_i} epsilon: ' + str(dqn_pp_agent.exploration_method.epsilon))
